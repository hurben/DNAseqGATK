...
def annotation(normal_file_format,input_normal_file, program_dir):
	...
	file_name_token = input_normal_file.split('/')
	file_name = file_name_token[len(file_name_token) -1]
	...
	if file_format == 'VCF':
		...
		print '\n>> EXECUTING ANNOVAR.'
		annovar_cmd = annovar_dir + '/' + 'annotate_variation.pl -out ' + str(file_name)+ '.annotation -buildver hg19 temp.annovar_ready ' + str(annovar_dir) +'/humandb/'
...
def annotation_file_to_dict(file_name):
	
	#open and readlines
	annotation_file = file(file_name)
...
if __name__ == '__main__':
	...
	for i in range(len(file_list_readlines)):
		...
		file_name_anot = str(file_name) + '.annotation.variant_function'
		
		annotation_dict = annotation_file_to_dict(file_name_anot)
